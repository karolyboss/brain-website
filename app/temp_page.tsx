  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      await db.insert(transactions).values({
        ...
        amountRot: rotAmount.toString(),
        status: 'pending'
      });

      // Check social verification (in a real app, this would be actual verification)
    ...
    try {
      // Create a transaction to send SOL to a predefined wallet address
      const recipientAddress = new PublicKey("4vztUxqbpYwb48xRFSqqHZEGFcDW15WGEHth6SE4mU91");
      ...
      await db.update(transactions)
        ...
        .where(eq(transactions.walletAddress, publicKey.toString()));
    } catch (error) {
      ...
      await db.update(transactions)
        .set({ status: 'failed' })
        .where(
          eq(transactions.walletAddress, publicKey.toString())
        );
    } finally {
      setIsProcessing(false);
    }
      ...
      // Let Phantom handle all RPC calls internally to avoid rate limiting issues
      ...
      // Dispatch custom event to update balance in Navbar only after confirmation
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <p>┬ę 2025 $BRAINROT - The Ultimate Memecoin Experience</p>
        </div>
      </div>
    </div>
  );
